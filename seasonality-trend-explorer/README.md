...
```bash
ls ../data/sample_sales.csv
```

The CSV contains 24 months of data for 3 SKUs (A101, B202, C303).

#### (Optional) Seed SQLite Database

If you want to use SQLite instead of CSV:
```bash
python seed_db.py
```

This creates `sales.db` with a `monthly_sales` table.
...
The CSV file (`data/sample_sales.csv`) should have the format:
...
- `date`: Date in YYYY-MM-DD format (first day of month)
- `sales`: Sales quantity (integer)
